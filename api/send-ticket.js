...
module.exports = async function handler(req, res) {
  ...
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(JSON.stringify({ id, p: plateau, ts: timestamp }))}&bgcolor=0A0806&color=E8820C&margin=8`;
  ...
  try {
    ...
    await resend.emails.send({
      from: 'Marseille Roots Comedy <billets@marseillerootscomedy.fr>',
      to: email,
      ...
      <div class="container">
        ...
        <div class="ticket">
          ...
          <div class="ticket-body">
            <div class="label">Date</div><div class="value">Samedi 4 Juillet 2026</div>
            ...
            <div class="label">Places</div><div class="value" style="font-size:28px;color:#E8820C">${qty}</div>
            <div class="label">Total</div><div class="value">${(price / 100).toFixed(2).replace('.', ',')} €</div>
            <div class="qr-block">
              <img src="${qrUrl}" width="180" height="180" alt="QR Code" style="border-radius:8px"/>
              <div class="ticket-id">${id}</div>
            </div>
          </div>
        </div>
        <div class="info">📱 <strong>Présentez ce QR code à l'entrée</strong> — sur téléphone ou imprimé.</div>
        <p style="font-size:13px;color:#8A7560">🍕 <strong style="color:#F5F0E8">Pizzeria Maison NPLTN</strong> & <strong style="color:#F5F0E8">Bar L'Angelis</strong> ouverts toute la soirée. DJ pour l'after !</p>
        <div class="footer">Marseille Roots Comedy · L'Arlequin · 13014 Marseille<br/>MAISON NPLTN · L'Angelis · L'Arlequin</div>
      </div></body></html>`,
    });
